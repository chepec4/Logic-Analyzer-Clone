...
#if !DBG
: NoOp {
    NoOp warn, full, setLevel, setOutput;
#else
{
    #define out(L, F) va_list A; va_start(A, F); out(L, F, A); va_end(A);
...
private:
    struct Options { int level; int output; char prefix[16]; int prefixSize; };
    static Options& options() { static Options aux = {Warning, Default, "", 0}; return aux; }
#endif

public:
    enum { None = 0, Error, Warning, Full, Killing = None };
    enum { Sys = 0, Std, Err, Default = Sys };
    trace() {}
} const trace;
