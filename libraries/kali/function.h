...
{
    typedef R (A::*F)(T, U);
    FuncAux(A* obj, F func, U user) : obj(obj), func(func), user(user) {}
...
struct Function
{
    ...
    template <typename U> void to(R (*f)(T, U), U u) {to(f ? new FuncAux<T, R (*)(T, U)>(f, u) : nullptr);}
    template <typename A, typename B> void to(A* a, R (B::*f)()) {to(new FuncAux<T, R (B::*)()>(a, f));}
    template <typename A, typename B> void to(A* a, R (B::*f)(T)) {to(new FuncAux<T, R (B::*)(T)>(a, f));}
    template <typename A, typename B, typename U> void to(A* a, R (B::*f)(T, U), U u) {to(new FuncAux<T, R (B::*)(T, U)>(a, f, u));}
...
struct Ftor : Function <R, T>
{
    ...
    template <typename A, typename U> Ftor(A* a, R (A::*f)(T, U), U u) {to(a, f, u);}
};
